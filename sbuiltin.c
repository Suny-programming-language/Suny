...
struct Sobj* Snumber(struct Sframe* frame) {
    ...
    Sgc_dec_ref(string, frame->gc_pool);
...
struct Sobj *Sload_dll(struct Sframe *frame) {
    ...
    Sgc_dec_ref(func, frame->gc_pool);
    Sgc_dec_ref(dll, frame->gc_pool);
...
struct Sobj* Spop(struct Sframe* frame) {
    ...
    dec_ref(last);
    Sgc_dec_ref(last, frame->gc_pool);
...
struct Sobj *Sint(struct Sframe *frame) {
    ...
    struct Sobj *obj = Sobj_set_int(value_int);
